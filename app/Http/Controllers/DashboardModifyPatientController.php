...
class DashboardModifyPatientController extends Controller {
    /**
    ...
    public function index(int $patient_id) {
        ...
        return view("dashboard_modify_patient", [
            "account" => $account,
            "patient" => $patient,
     ...
     * @return string
     */
    public function modifyPatient(int $patient_id, Request $request) {
        $patient = Auth::user()->getPatient($patient_id);
        if ($patient == null) {
        ...
        if ($request->hasFile("picture_file")) {
            $oldPictureFileRoute = $patient->picture_file;
            $patient->picture_file = $request->file("picture_file")->store(
                "", "patientPictureFiles"
            );
            if ($oldPictureFileRoute != null) {
                Storage::disk("patientPictureFiles")->delete($oldPictureFileRoute);
            }
        }
        try {
            DB::beginTransaction();
