...
<script>
    $(document).ready(function () {
        ...
        $(".ui.checkbox").checkbox({
            beforeChecked: function (element) {
                return false;
            }
        });
...
<div class="dashboard_page_header">
    <h2 class="ui left floated dividing header">
        {{ __("dashboard.doing_clinical_record_of", ["doing" => __("dashboard.consulting"), "clinical_record_name" => $clinicalRecord->name]) }}
        <div class="sub header">{{ __("dashboard.of_name", ["full_name" => $patient->getFullName()]) }}</div>
    </h2>
    <a href="#patient/{{ $patient->patient_id }}/clinicalRecord/{{ $clinicalRecord->clinical_record_id }}/modify" 
        class="ui right floated primary labeled icon button">
        <i class="edit icon"></i>
                ...
                @switch($field->type)
                    @case(App\Enums\ClinicalRecordFieldType::INPUT_TEXT)
                        <input type="text" name="field_{{ $field->clinical_record_field_id }}" 
                            maxlength="500" autocomplete="off" value="{{ $field->getValue($patient->patient_id) }}" readonly/>
                        @break
                    @case(App\Enums\ClinicalRecordFieldType::TEXT_AREA)
                        <textarea name="field_{{ $field->clinical_record_field_id }}" rows="3" maxlength="500" readonly>{{ $field->getValue($patient->patient_id) }}</textarea>
                        @break
                                ...
                                <div class="field">
                                    <div class="ui radio read-only checkbox">
                                        <input type="radio" name="field_{{ $field->clinical_record_field_id }}" 
                                            value="{{ $option->clinical_record_field_option_id }}" readonly {{ 
                                            $selectedOption != null && 
